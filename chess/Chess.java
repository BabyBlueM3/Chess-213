...
public class Chess {
    ...
    public static ReturnPlay play(String move) {
		...
		if (parts.length == 2 || (parts.length == 3 && parts[2].equals("draw?"))) {
			...
			if (piece == null || !isCurrentPlayerPiece(piece) || !isValidMove(piece, parts[1], false) || isCheck(false)) {
    ...
    private static boolean isCheck(boolean isMate) {
		...
		if(isMate) { // Check if king has any legal moves by moving it to every possible position and seeing if its in check.
				...
				for (int f = -1; f <= 1; f++) { // f for File
					...
					if (Character.getNumericValue(king.toString().charAt(1)) + r >= 1 && Character.getNumericValue(king.toString().charAt(1)) + r <= 8) {
						...
						if (isValidMove(king, destination, false)) { // If the king is able to survive at least one potential move, it is not in checkmate
							...
							for (ReturnPiece piece : board) {
								System.out.print("\npiece: " + piece); //										debug
								if (piece.pieceType.toString().charAt(0) != kingType.toString().charAt(0) && isValidMove(piece, destination, true)) {
									isSafe = false;
			...
			for (ReturnPiece piece : board) {
				...
				if (piece.pieceType.toString().charAt(0) != kingType.toString().charAt(0) && isValidMove(piece, king.pieceFile.toString() + king.pieceRank, true)) {
